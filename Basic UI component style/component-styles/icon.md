...
本文定义后台产品中的图标使用规则。AI 生成 HTML Demo、检查 HTML 可演示页面或整理设计稿时，遇到 icon 必须优先遵循本文。
...
```text
//at.alicdn.com/t/c/font_5177816_5ilr2y8ewpn.css
```
...
@font-face {
  ...
  src: url('//at.alicdn.com/t/c/font_5177816_5ilr2y8ewpn.woff2?t=1779701125662') format('woff2'),
       url('//at.alicdn.com/t/c/font_5177816_5ilr2y8ewpn.woff?t=1779701125662') format('woff'),
       url('//at.alicdn.com/t/c/font_5177816_5ilr2y8ewpn.ttf?t=1779701125662') format('truetype');
}
...
> 例外：数据指标卡中的 3D 图标不使用 iconfont，必须遵循 `data-metric-card.md`，且只能从 `assets/3Dicon/` 选取已有素材。该例外不扩展到菜单、按钮、表格、状态、空状态或弹层标题。
...
| 表格字段配置 / 列设置 | `icon-a-shezhixitong` |
| 筛选 | `icon-shaixuan` |
...
- 图标颜色默认使用 `#8A9099`。
- 用于标题前方展开 / 收起的内联 icon 应与 Select 下拉箭头风格一致：透明背景、无边框、无阴影；hover / focus / active 不改变背景，不增加底部投影，悬停时仅 icon 颜色变化。
...
- 数据指标卡如使用 3Dicon，必须回看 `data-metric-card.md`，不得按普通 iconfont 规则自行替换。
- 不建议在长期交付场景直接依赖在线 iconfont 链接，应使用本地 `assets/iconfont/` 资源或交付方确认的字体包。
...
- 禁止把 `assets/3Dicon/` 中的 3D 图标挪用到数据指标卡以外的普通 icon 场景。
