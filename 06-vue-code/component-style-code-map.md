...
Keywords: component mapping, html demo, vue codegen, component dom extraction, ant design vue, token

本文用于把 `02-components/` 中的组件语义规范，映射到 HTML Demo 的真实组件 DOM / class / CSS / Token，以及 Vue 3 + TypeScript + Ant Design Vue 页面代码。
...
- HTML Demo 可以不真实引入 Ant Design Vue，但必须使用组件样式库中的真实 DOM、状态节点和样式。
...
| 规范文档 | Vue 组件 | HTML Demo 来源 | Token / 样式来源 | 必须状态 |
|---|---|---|---|---|
| `button.md` | `a-button` | 组件样式库 Button DOM / class | 组件样式库 Button CSS / Token | default / hover / active / disabled / loading / danger |
| `input.md` | `a-input`、`a-input-search` | 组件样式库 Input DOM / class | 组件样式库 Input CSS / Token | default / hover / focus / filled / disabled / error |
| `textarea.md` | `a-textarea` | 组件样式库 Textarea DOM / class | 组件样式库 Textarea CSS / Token | focus / disabled / error / count |
| `select.md` | `a-select`、`a-tree-select`、`a-cascader` | 组件样式库 Select DOM / class | 组件样式库 Select CSS / Token | open / hover / selected / disabled / loading / empty |
| `checkbox.md` | `a-checkbox`、`rowSelection` | 组件样式库 Checkbox DOM / class | 组件样式库 Checkbox CSS / Token | unchecked / checked / indeterminate / disabled |
| `radio.md` | `a-radio`、`a-radio-group` | 组件样式库 Radio DOM / class | 组件样式库 Radio CSS / Token | unchecked / checked / disabled / focus |
| `switch.md` | `a-switch` | 样式库待补齐，需最小补充样式 | on `p6` | off / on / loading / disabled |
| `tabs.md` | `a-tabs` | 组件样式库 Tabs DOM / class | 组件样式库 Tabs CSS / Token | default / hover / active / disabled |
| `table.md` | `a-table` | 组件样式库 Table DOM / class | 组件样式库 Table CSS / Token | loading / empty / hover / selected / error |
| `pagination.md` | `a-pagination` | 组件样式库 Pagination DOM / class | 组件样式库 Pagination CSS / Token | default / hover / active / disabled |
| `tag.md` | `a-tag` | 组件样式库 Tag DOM / class | 组件样式库 Tag CSS / Token | default / status / removable |
| `alert.md` | `a-alert` | 样式库待补齐，需最小补充样式 | info / success / warning / error Token | info / success / warning / error / closable |
| `toast-message.md` | `message`、`notification` | 组件样式库 Toast DOM / class | 组件样式库 Toast CSS / Token | info / success / warning / error |
...
代码或 HTML Demo 必须包含查询表单、表格、分页、状态标签、行操作、批量选择、loading / empty / error 和高风险操作确认。HTML Demo 中这些组件的 DOM、CSS、Token 必须来自组件样式库。
...
代码或 HTML Demo 必须包含表单分组、输入、选择、单选、多选、开关、必填、校验、submit loading、validation error、success / error feedback 和高风险配置确认。
...
代码或 HTML Demo 必须包含详情容器或 Drawer、对象摘要、Descriptions / Tabs / Timeline / Table、状态 Tag、操作按钮和高风险操作确认。Drawer 若样式库未提供稳定 DOM，应按待补齐组件处理。
...
- `06-vue-code/component-dom-extraction-rules.md` 负责约束组件 DOM 必须从组件样式库抽取。
- `06-vue-code/component-style-import-rules.md` 负责约束组件 CSS 与 Token 必须注入。
- `06-vue-code/business-component-reuse-rules.md` 负责约束业务区组件必须复用真实 DOM / class / CSS / Token。
- `06-vue-code/deprecated-class-blacklist.md` 负责集中维护旧别名和私有组件 class 清单。
...
HTML Demo 不强制真实引入 Ant Design Vue，但必须复用组件样式库中的真实 DOM、状态表现、Token 色值、交互反馈、mock 数据、loading / empty / error 和高风险确认操作。
