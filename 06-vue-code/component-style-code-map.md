...
Keywords: component mapping, ant design vue, html demo, css token, style implementation, codegen
...
| 规范文档 | Vue 组件 | HTML Demo 结构 | 样式 Token / class | 必须状态 |
|---|---|---|---|---|
| `button.md` | `a-button` | `.btn .btn-primary .btn-secondary .btn-danger` | 普通按钮 `p6/p5/p7`，AI 使用原 AI Token | default / hover / active / disabled / loading / danger |
| `input.md` | `a-input`、`a-input-search` | `.form-input .search-input` | border `gray-5`，focus `p6 + p1 ring` | default / hover / focus / filled / disabled / error |
| `textarea.md` | `a-textarea` | `.form-textarea` | 同 Input，高度按场景 | focus / disabled / error / count |
| `select.md` | `a-select`、`a-tree-select`、`a-cascader` | `.select .select-panel .select-option` | selected `p1 + p7`，focus `p6` | open / hover / selected / disabled / loading / empty |
| `checkbox.md` | `a-checkbox`、`rowSelection` | `.checkbox` | checked `p6` | unchecked / checked / indeterminate / disabled |
| `radio.md` | `a-radio`、`a-radio-group` | `.radio .radio-button` | checked `p6` | unchecked / checked / disabled / focus |
| `switch.md` | `a-switch` | `.switch` | on `p6` | off / on / loading / disabled |
| `tabs.md` | `a-tabs` | `.tabs .tab-item` | active `p6/p7` | default / hover / active / disabled |
| `table.md` | `a-table` | `.data-table` | header `gray-2/3`，border `gray-4` | loading / empty / hover / selected / error |
| `pagination.md` | `a-pagination` | `.pagination` | active `p6` | default / hover / active / disabled |
| `tag.md` | `a-tag` | `.tag .tag-status` | 主色 `p1 + p7`，语义色按状态 | default / status / removable |
| `alert.md` | `a-alert` | `.alert` | info/success/warning/error 语义色 | info / success / warning / error / closable |
| `toast-message.md` | `message`、`notification` | `.toast` | 语义色 icon + 文案 | info / success / warning / error |
| `motion.md` | CSS transition | `transition` class | 150ms / 200ms / 300ms | hover / open / close / loading |
