...
本文用于把 `02-components/` 中的组件语义规范，映射到 Vue 3 + TypeScript + Ant Design Vue 页面代码和单文件 HTML Demo 实现。
...
其中 `backend_ai_ui_component_kit_with_index.html` 是真实可运行组件样式库。HTML Demo 的组件 class 必须以该文件中真实存在的 `.ant-*` class 为准。
...
生成结果中不得使用该黑名单文件列出的 class 作为基础组件 class。
...
- HTML Demo 基础组件 class 必须使用 `.ant-*` 体系。
...
- 旧 class 检查统一依据 `06-vue-code/deprecated-class-blacklist.md`。
...
代码或 HTML Demo 必须包含查询表单、表格、分页、状态标签、行操作、批量选择、loading / empty / error 和高风险操作确认。
...
代码或 HTML Demo 必须包含详情容器或 Drawer、对象摘要、Descriptions / Tabs / Timeline / Table、状态 Tag、操作按钮和高风险操作确认。
...
- `06-vue-code/business-component-reuse-rules.md` 负责约束业务区组件必须复用真实基础 class。
- `06-vue-code/component-style-import-rules.md` 负责约束最终 HTML 必须注入所用组件 CSS。
- `06-vue-code/deprecated-class-blacklist.md` 负责集中维护旧别名和私有组件 class 黑名单。
...
HTML Demo 不强制真实引入 Ant Design Vue，但必须模拟同等组件结构、状态表现、Token 色值、交互反馈、mock 数据、loading / empty / error 和高风险确认操作。
...
禁止使用的旧别名 class 和页面私有组件 class 统一见：

```text
06-vue-code/deprecated-class-blacklist.md
```

当生成内容涉及 Button、Tabs、Input、Select、Checkbox、Radio、Table、Tag、Tooltip、DatePicker、Pagination 等基础组件的细节样式时，应参考 `docs/component-style-library/` 中的完整 HTML Demo，并同时遵循 `06-vue-code/business-component-reuse-rules.md`、`06-vue-code/component-style-import-rules.md` 和 `06-vue-code/deprecated-class-blacklist.md`。该 Demo 用于校验视觉和状态，不替代 `06-vue-code/templates/` 页面框架母版。
...
- 是否读取并执行了 `06-vue-code/component-style-import-rules.md`？
- 是否读取并执行了 `06-vue-code/deprecated-class-blacklist.md`？
- 普通主色是否使用 `p6 #00AB7A`？
- AI 渐变是否保持原 Token？
- 是否存在用 div 伪造基础组件？
